...
public class ChatClient extends JFrame implements Runnable {
	...
	public ChatClient() {
		...
		img.setIcon(new ImageIcon(ChatClient.class.getResource("/Images/IMG_5856.JPG")));
		...
		center_p.add(new JLabel("방제"), BorderLayout.NORTH);

		room_list = new JList();
		room_list.setFont(new Font("맑은 고딕", Font.PLAIN, 20));
		...
		login_bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (name.length() > 0) {
					...
					JOptionPane.showMessageDialog(getParent(), "닉네임을 입력해주세용");
					nickname_tf.setText("");
		...
		memo_bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					String msg = JOptionPane.showInputDialog("내용을 입력하세용");
					if (msg.length() <= 0 || msg == null) {
						JOptionPane.showMessageDialog(getParent(), "내용을 입력하세용~");
					} else {
						Protocol p = new Protocol();
						p.setCmd(6);
						p.setMsg(msg);
		...
		mkRoom_bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 사용자에게 방 제목 받기
				String name = JOptionPane.showInputDialog(getParent(), "방제를 입력하세용");
				if (name.length() <= 0 || name == null) {
					JOptionPane.showMessageDialog(getParent(), "방제를 입력하세용");
				} else {
					try {
						Protocol p = new Protocol();
						p.setCmd(3);
	...
	private boolean connected() {
		boolean value = true;
		try {
			s = new Socket("192.168.0.91", 7779);
		...
		esc: while (true) {
			try {
				...
				if (obj != null) {
					...
					case 1: // 메시지 전달
						jta.append(p.getMsg() + "\n");
						jta.setCaretPosition(jta.getText().length()); // 추가된만큼 올라감?
						break;
					case 2: // 접속 및 갱신
						user_list.setListData(p.getNames()); // 대기자 명단
						room_list.setListData(p.getRooms()); // 방 목록
						break;
					case 4: // 방 참여
