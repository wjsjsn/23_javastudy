...
// 이벤트 리스너 : 이벤트를 감지, 이벤트가 발생되면 실제적인 이벤트를 처리할 수 있도록 만든 인터페이스
// 이벤트 핸들러 : 리스너에서 전달된 이벤트를 실제로 처리할 수 있도록 이벤트 리스너에 포함되어 있는 메서드(추상메서드)
//                       즉, 발생된 이벤트 객체를 받아서 실제적으로 처리해주는 기능을 하는 메서드

// ActionEvent : 컴포넌트를 눌렀거나 선택되었을 때 혹은 enter키를 눌렀을 때 발생하는 이벤트
// ActionListener : actionPerformed() 추상 메서드를 갖고 있음
...
public class Ex01_ActionEvent extends JFrame implements ActionListener{
	...
	public Ex01_ActionEvent() {
		...
		// 이벤트 소스에 ActionListener 붙이기
		// this를 사용한 이유 : 현재 객체가 ActionListener가 있음
	...
	@Override
		...
		if(obj == jb1) {
			System.out.println("입력 1이 눌림");
		}else if(obj == jb2) {
			System.out.println("입력2가 눌림");
		}else if(obj == jb3) {
		System.exit(0);
		}
	}
