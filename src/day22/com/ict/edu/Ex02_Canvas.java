...
public class Ex02_Canvas extends JFrame {
	...
	private class CanvasTest extends Canvas {
		...
		public void paint(Graphics g) {
			...
			// 채워진 둥근모서리 사각형 : fillRoundRect(시작위치, 높이, 넓이, 호넓이, 호높이)
			g.setColor(Color.GRAY);
			...
			// 글자 : drawString("글자", 시작위치(글자 밑 부분))
			g.setColor(Color.GRAY);			
			g.drawString("대한민국", 120, 170);
			g.setColor(Color.YELLOW);						
			g.drawLine(0, 170, 170, 170);
			g.drawLine(120, 0, 120, 220);
			...
			try {
				...
				g.drawImage(image2, 350, 230, 100, 100, new Color(255, 255, 0, 30), this);    // 크기 조절								
			} catch (Exception e) {
			}
			
			Image image3 = new ImageIcon("src/images/java3.png").getImage();
			g.drawImage(image3, 460, 230, 100, 100, new Color(0, 255, 0, 20), this);  
		}
	}
